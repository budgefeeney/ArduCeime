...
One could argue, given the different evaluation methodologies, that our results overfits the data, since it uses ID features which are the same in training and query datasets, and indeed it is remarkable that perplexity falls below 500. However even so, the query perplexity results for our model beat the training perplexity results for vanilla author-topic models, which absolutely are overfit.
