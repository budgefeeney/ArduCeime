...
\only<2-> {
    \begin{align*}
    \lse(\vv{\theta}) & \leq \half \vv{y}\T \Axi \vv{y} - \bxi\T\vv{y} + \cxi
    \end{align*}
}
...
\only<3> {
    ...
    A second order Taylor approximation where the Hessian is replaced with $B \succeq H$, where $B = \left( I - \frac{1}{K+1}\one \one\T \right)$
}
