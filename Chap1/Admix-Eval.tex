...
Between these two extremes, other metrics exist. The ``topical coherence" metric, which first introduced in \cite{Mimno2011} and additionally used in \cite{Mimno2012a}, directly addresses the word-intrusion problem. One can assume that if a set of words all belong to a single topic, one would see most of them occur in a single document belonging to that topic, and not see occur uniformly across all documents and all topics. Therefore letting $D(t)$ be the number of documents where word $t$ occurs at least once, and $D(t,v)$ be the number of documents containing at least once occurrence each of both $t$ and $v$, the coherence metric is defined as
\begin{align}
...
With labelled data, specified with a 1-of-$L$ labelling $\vv{t}_d$, one may choose to evaluate the model by comparing it to the inferred topics, $\thd \in \VReal{K}$, where $K$ need not necessarily equal $L$. Many metrics exist for this purporse, of which the most well known is the F1-Score which is is the harmonic mean of precision, $p$ and recall, $r$, $F_1 = 2 \frac{p \cdot r}{p + r}$ where precision is $p=\frac{TP}{TP+FP}$ and $r=\frac{TP}{TP+FN}$ and $TP, FP, TN, FN$ refer to the true and false positives and negatives respectively.
