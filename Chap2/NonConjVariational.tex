...
where $y_n$ is our predicted target value and $w \in \VReal{D}$ is a vector of weights\footnote{We assume the usual setup where the final element of $\vv{x}$ is fixed at one so the final element of $\vv{w}$ can act as the intercept}. However this model obviously will not work, as $y_n$ is in no way constrained to lie in $[0 \ldots 1]$.
